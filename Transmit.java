...
public class Transmit implements Serpentable {

	private String input; // 16 letters
	private String key; // 32 letters
	// private String[] keys;
	private String a;
	private String b;
	private String c;
	...
	public Transmit(String message, String k) {
		input = message;
		/*input = "0" + new BigInteger(input.getBytes()).toString(2);
		...
		if (round != 33) {
			...
			for (int h = 0; h < input.length(); h += 4) { // sbox baby
				for (int s = 0; s < sbox1.length; s++) {
					if (input.substring(h, h + 4).equals(sbox1[s]))
						input = input.substring(0, h) + choose(round)[s] + input.substring(h + 4);
				}
			}
	...
	public void encrypt() {
		...
		String output = "";
		// System.out.println(input.length());
		System.out.println("binary code: "+input);
	...
	@Override
	public void decrypt() {
		Receive test = new Receive(input, key);
		test.decrypt();
	}
